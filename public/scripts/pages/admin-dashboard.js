    ...
    function renderRequestsSection() {
      ...
      list.innerHTML = rows.map((row) => {
        ...
        const hospital = patient?.hospital_name || 'Unknown facility';
        const status = normalizeRequestStatus(row.status);
        ...
        const actionItems = actions
          .map((target) => `<button type="button" class="request-action-menu-item" onclick="openRequestStatusModal(${Number(row.request_id)}, '${target}')">${getActionLabel(target)}</button>`)
          .join('');
        ...
        return `<div class="request-card">
          <div class="request-type">${escapeHtml(row.blood_type_needed || '-')}</div>
