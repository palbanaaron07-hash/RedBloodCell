...
create table if not exists blood_bank.request_verification_support (
  request_id bigint primary key references blood_bank.blood_request(request_id) on delete cascade,
  facility_contact varchar(120),
  storage_path text,
  ...
  constraint request_verification_support_file_check check (
    (storage_path is null and file_name is null and mime_type is null and file_size is null)
    or
    (storage_path is not null and file_name is not null and mime_type in ('image/jpeg', 'image/png', 'application/pdf')
      and file_size > 0 and file_size <= 5242880)
  ),
  constraint request_verification_support_method_check check (
...
create policy request_supporting_documents_delete_owner
  on storage.objects
  for delete to authenticated
  using (
    bucket_id = 'request-supporting-documents'
...
begin
  ...
  from blood_bank.request_verification_support
  where request_id = p_request_id;
...
-- Retire the earlier entry point so verification evidence cannot be skipped.
