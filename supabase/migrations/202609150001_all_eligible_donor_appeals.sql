-- Show every currently eligible donor for an approved community request.
-- The authorized blood facility, rather than this application, makes the final match.

begin;
set search_path to blood_bank, public;

-- Keep the migration compatible with deployments that have not yet received
-- the requester-exclusion helper migration.
...
begin
  ...
  select
    ...
    'donor_appeal',
    'Blood donation request available',
    'Request #' || request_row.request_id || ' needs ' || request_row.quantity ||
      ' donor unit(s). Open VeinDrop to review the request. The authorized blood facility will make the final donor match.'
  from blood_bank.donor d
  where d.auth_user_id is not null
    and lower(coalesce(d.availability_status, '')) = 'available'
    and lower(coalesce(d.donor_status, 'registered')) in ('approved', 'donated')
  ...
  insert into blood_bank.blood_request_status_log (
    ...
    inserted_count || ' new in-app alert(s); ' || eligible_count || ' eligible donor(s) notified.',
    now()
  );
...
begin
  ...
  if not found or request_row.verification_status <> 'verified' or request_row.status <> 'approved' then
